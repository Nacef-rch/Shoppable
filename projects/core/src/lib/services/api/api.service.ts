...
import { handleError, handleSuccess } from '@core/helpers/api-helpers';
...
export class ApiService {
    ...
    public get(url: string): Observable<any> {
        return this.http
            .get(`${environment.firebaseAPIKey}${url}`, {
    ...
    public put(url: string, data: any, extendOption?: any): Observable<any> {
        ...
        return this.http
            .put(`${environment.firebaseAPIKey}${url}`, JSON.stringify(data), options)
            .pipe(
                map((res) => handleSuccess(res)),
                catchError((res) => handleError(res))
            );
    ...
    public delete(url: string): Observable<any> {
        return this.http
            .delete(`${environment.firebaseAPIKey}${url}`, {
                headers: this.headers,
                observe: 'response'
            })
            .pipe(
